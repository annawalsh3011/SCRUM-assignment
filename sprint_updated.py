...
basket_list = []
def add_to_basket(items, quantity, stock, type):
    if quantity < stock:
        basket_list.append(items, type, quantity)
    else:
        print('Not enough stock to complete this order')

def sum_basket(basket_list):
    for i in basket_list:
        count = 0
        if basket_list[i] == 'Luxury':
            vat = 50 * .2
            count = count + 50 + vat
        if basket_list[i] == 'Gift':
            vat = 20 * .05
            count = count + 20 + vat
        if basket_list[i] == 'Essential':
            vat = 30 * .1
            count = count + 30 + vat
    return count